...
output "products_public_ips" {
  description = "Public IP addresses for the products service instances"
  value       = { for id, instance in aws_instance.products : id => instance.public_ip }
}

# Salida. Muestra las direcciones IP privadas de las instancias de la aplicación de alarmas.
output "products_private_ips" {
  description = "Private IP addresses for the products service instances"
